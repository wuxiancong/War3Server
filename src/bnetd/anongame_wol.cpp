        ...
        if ((player1 != player2) && ((conn_get_channel(conn_pl1)) == (conn_get_channel(conn_pl2)))) {
            ...
            case CLIENTTAG_REDALERT2_UINT: {
                ...
                if (std::strcmp(channelname, RAL2_CHANNEL_FFA) == 0) {
                    ...
                    _send_msg(conn_pl1, "PRIVMSG", temp.c_str());
                    _send_msg(conn_pl2, "PRIVMSG", temp.c_str());
                }
                else
...
static int anongame_wol_tokenize_line(t_connection * conn, char const * text)
{
    ...
    line = (char *)xmalloc(std::strlen(text) + 16);
    std::size_t text_len = std::strlen(text);
    line = (char *)xmalloc(text_len + 16);
    std::memcpy(line, text, text_len + 1);
    ...
    *temp++ = '\0';
    ...
    if ((std::strcmp(command, "Match") == 0)) {
        std::size_t tlen = std::strlen(temp);
        temp[tlen] = ',';
        temp[tlen + 1] = '\0';

        /* FIXME: This is DUMB - without that we lost the last tag/param */
