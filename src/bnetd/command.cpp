...
static int _handle_swapself_bot_command(t_connection * c, char const * text);
...
static const t_command_table_row standard_command_table[] =
{
    ...
    { "/swapself",      _handle_swapself_bot_command },
...
static int _handle_clan_command(t_connection * c, char const * text)
{
    ...
    else
    {
        if ((args[1] == "create" || args[1] == "cre"))
        {
            ...
            if (clanlist_find_clan_by_clantag(str_to_clantag(clantag))) {
...
static int _glist_cb(t_game *game, void *data)
{
    ...
    if (!cbdata->tag)
    {
        size_t current_len = std::strlen(msgtemp0);
        if (current_len < sizeof(msgtemp0) - 1) {
            std::strncat(msgtemp0, clienttag_uint_to_str(game_get_clienttag(game)), sizeof(msgtemp0) - current_len - 1);
        }

        current_len = std::strlen(msgtemp0);
        if (current_len < sizeof(msgtemp0) - 1) {
            std::strncat(msgtemp0, " ", sizeof(msgtemp0) - current_len - 1);
        }
    }
    ...
    if ((!prefs_get_hide_addr()) || (account_get_command_groups(conn_get_account(cbdata->c)) & command_get_group("/admin-addr"))) /* default to false */
    {
        size_t current_len = std::strlen(msgtemp0);
        if (current_len < sizeof(msgtemp0) - 1) {
            std::strncat(msgtemp0, addr_num_to_addr_str(game_get_addr(game), game_get_port(game)), sizeof(msgtemp0) - current_len - 1);
        }
    }
                ...
                )
        {
            ...
            for (conn = channel_get_first(channel); conn; conn = channel_get_next())
            {
                        ...
                        channel_conn_is_tmpOP(channel, account_get_conn(acc)))
                {
                    ...
                    if (!first) {
                        remaining = sizeof(msgtemp0) - std::strlen(msgtemp0) - 1;
                        std::strncat(msgtemp0, " ,", remaining);
                    }

                    remaining = sizeof(msgtemp0) - std::strlen(msgtemp0) - 1;
                    std::strncat(msgtemp0, name, remaining);

                    if (account_get_auth_admin(acc, NULL) == 1) {
                        remaining = sizeof(msgtemp0) - std::strlen(msgtemp0) - 1;
                        std::strncat(msgtemp0, "(A)", remaining);
                    }
                    else if (account_get_auth_operator(acc, NULL) == 1) {
                        remaining = sizeof(msgtemp0) - std::strlen(msgtemp0) - 1;
                        std::strncat(msgtemp0, "(O)", remaining);
                    }
                    else if (account_get_auth_admin(acc, channel_get_name(channel)) == 1) {
                        remaining = sizeof(msgtemp0) - std::strlen(msgtemp0) - 1;
                        std::strncat(msgtemp0, "(a)", remaining);
                    }
                    else if (account_get_auth_operator(acc, channel_get_name(channel)) == 1) {
                        remaining = sizeof(msgtemp0) - std::strlen(msgtemp0) - 1;
                        std::strncat(msgtemp0, "(o)", remaining);
                    }
                    first = 0;
...
static int _handle_connections_command(t_connection *c, char const *text)
{
    ...
    LIST_TRAVERSE_CONST(connlist(), curr)
    {
        ...
        {
            channel_name_str = channel_get_name(conn_get_channel(conn));
        }
        else
        {
            channel_name_str = localize(c, "none");
        }
...
static int _handle_gameinfo_command(t_connection * c, char const *text)
{
...

    {
        ...
        if (!conn_get_account(game_get_owner(game)))
        {
            tname = NULL;
    ...
    {
        char const * mapname = game_get_mapname(game);
        std::string mapnameStr;

        if (!mapname)
            mapnameStr = localize(c, "unknown");
        mapname = mapnameStr.c_str();
...
static int _handle_swapself_bot_command(t_connection * c, char const * text)     { return _handle_bot_proxy_logic(c, text); }
