...
extern int friendlist_add_account(t_list * flist, t_account * acc, int mutual)
{
    ...
    t_friend * fr = new (std::nothrow) t_friend;
    if (!fr) {
        eventlog(eventlog_level_error, __FUNCTION__, "out of memory: cannot create friend entry");
        return -1;
    }
    ...
    if (list_append_data(flist, fr) < 0) {
        eventlog(eventlog_level_error, __FUNCTION__, "failed to append friend to list");
        delete fr;
        return -1;
    }
