...
extern int game_set_pass(t_game * game, char const * pass)
{
    ...
    if (!pass) {
        eventlog(eventlog_level_error, __FUNCTION__, "got NULL pass");
        return -1;
    }
...
extern int game_set_info(t_game * game, char const * info)
{
    ...
    if (game->info) xfree((void *)game->info);
    game->info = xstrdup(info);

    /* 核心：重新解析 StatString，更新内存中的房主名、地图名、人数等 */
    game_parse_info(game, info);
...
extern void game_set_status(t_game * game, t_game_status status)
{
    ...
    if (game->status == game_status_started &&
        ...
        // 仅记录日志，不再 return，允许房主迁移时的状态回退
        eventlog(eventlog_level_info, "game_set_status",
                 "Allowing status roll-back from '{}' to '{}' for game \"{}\"",
                 game_status_get_str(game->status), game_status_get_str(status), game_get_name(game));
    }
    else if (game->status == game_status_done && status != game_status_done) {
        eventlog(eventlog_level_error, "game_set_status",
                 "attempting to set status '{}' ({}) to done game", game_status_get_str(status), status);
        return;
    }
