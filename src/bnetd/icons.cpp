    ...
    if (!((master_commandgroups & account_get_command_groups(conn_get_account(c)))))
    {
        ...
        if (char const * iconstash = account_get_user_iconstash(account, clienttag))
        {
            ...
            do
            {
                ...
                std::string safe_alias;

                // 1. Resolve Alias (Updates display text if found)
                const char* stash_alias = customicons_stash_find(clienttag, _icon.c_str(), true);
                if (stash_alias) {
                    _icon = std::string(stash_alias);
                    safe_alias = stash_alias;
                } else {
                    safe_alias = _icon;
                }

                // 2. Resolve Code (Using updated _icon)
                const char* stash_code = customicons_stash_find(clienttag, _icon.c_str());
                if (stash_code) {
                    safe_code = stash_code;
                } else {
                    // Not in stash, fallback to current string
                    safe_code = _icon;
                    safe_alias = _icon;
                }

                // 3. Check Selection (Compare against safe copy)
                if (usericon && strcasecmp(usericon, safe_code.c_str()) == 0)
                {
                    _icon = "[" + _icon + "]";
                    is_selected = true;
                }
                ...
                // 4. Check Search Parameter (Compare against safe copies)
                if (iconname[0] != '\0')
                {
                    if (strcasecmp(iconname, safe_alias.c_str()) == 0 || strcasecmp(iconname, safe_code.c_str()) == 0)
                        is_found = true;
                }
    ...
    case 'l':
        ...
        if (char const * iconstash = account_get_user_iconstash(account, clienttag))
        {
            ...
            do
            {
                ...
                // 1. Resolve Code (Must do before modifying _icon)
                const char* stash_code = customicons_stash_find(clienttag, _icon.c_str());
                if (stash_code) {
                    safe_code = stash_code;
                } else {
                    safe_code = _icon; // Fallback
                }

                // 2. Resolve Alias (Updates display text)
                const char* stash_alias = customicons_stash_find(clienttag, _icon.c_str(), true);
                if (stash_alias) {
                    _icon = std::string(stash_alias);
                }

                // 3. Check Selection (Use safe_code, ignore _icon modification)
                if (usericon && strcasecmp(usericon, safe_code.c_str()) == 0)
                {
                    _icon = "[" + _icon + "]";
                    is_selected = true;
    ...
    for (line = 1; (buff = file_get_line(fp)); line++)
    {
        ...
        if ((option = _read_option(buff, line)))
        {
            if (std::strcmp(option->key, "custom_icons") == 0) {
                enable_custom_icons = std::strcmp(option->value, "true") == 0 ? 1 : 0;
            }
