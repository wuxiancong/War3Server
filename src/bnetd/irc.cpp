...
extern int irc_send_cmd(t_connection * conn, char const * command, char const * params)
{
    ...
    if (params)
        len = std::snprintf(data, sizeof(data), ":%s %s %s %s", ircname, command, nick, params);
    else
        len = std::snprintf(data, sizeof(data), ":%s %s %s", ircname, command, nick);
...
extern int irc_send_ping(t_connection * conn)
{
    ...
    if (conn_get_state(conn) == conn_state_bot_username)
        len = std::snprintf(data, sizeof(data), "PING :%u", conn_get_ircping(conn));
    else
        len = std::snprintf(data, sizeof(data), "PING :%s", server_get_hostname());

    if (len < 0 || (size_t)len >= sizeof(data))
...
extern int irc_send_pong(t_connection * conn, char const * params)
{
    ...
    if (params)
        len = std::snprintf(data, sizeof(data), ":%s PONG %s :%s", server_get_hostname(), server_get_hostname(), params);
    else
...
extern int irc_message_postformat(t_packet * packet, t_connection const * dest)
{
    ...
    *e4++ = '\0';
    ...
    if (std::strlen(e1) + 1 + std::strlen(e2) + 1 + std::strlen(toname) + std::strlen(temp) + std::strlen(e4) + 2 + 1 <= MAX_IRC_MESSAGE_LEN) {
        ...
        if (e1_2)
            len = std::snprintf(msg, sizeof(msg), "%s@hidden %s %s%s%s", e1, e2, toname, temp, e4);
        else
            len = std::snprintf(msg, sizeof(msg), "%s %s %s%s%s", e1, e2, toname, temp, e4);
...
int irc_send_rpl_namreply_internal(t_connection * c, t_channel const * channel) {
    ...
    int written = std::snprintf(temp, sizeof(temp), "%c %s :",
                                ((channel_get_permanent(channel)) ? ('=') : ('*')), ircname);
    ...
    if (!(channel_get_flags(channel) & channel_flags_thevoid)) {
        for (m = channel_get_first(channel); m; m = channel_get_next()) {
            ...
            if (flags & MF_BLIZZARD)
                std::snprintf(flg, sizeof(flg), "@");
            else if ((flags & MF_BNET) || (flags & MF_GAVEL))
                std::snprintf(flg, sizeof(flg), "%%");
            else if (flags & MF_VOICE)
                std::snprintf(flg, sizeof(flg), "+");

            std::memset(entry_buf, 0, sizeof(entry_buf));
            size_t e_off = 0;
            ...
            if (conn_get_wol(c) == 1) {
                ...
                if ((first) && ((std::strcmp(ircname, "#Lob_38_0") == 0) ||
                    ...
                    e_off += std::snprintf(entry_buf + e_off, sizeof(entry_buf) - e_off, "@matchbot,0,0 ");
                }

                if ((game) && (game_get_channel(game) == channel)) {
                    if (game_get_owner(game) == m) {
                        e_off += std::snprintf(entry_buf + e_off, sizeof(entry_buf) - e_off, "@");
                    }
                }
                else {
                    if ((flags & MF_BNET) || (flags & MF_GAVEL))
                        std::snprintf(flg, sizeof(flg), "@");
                    e_off += std::snprintf(entry_buf + e_off, sizeof(entry_buf) - e_off, "%s", flg);
                }
                ...
                if (tag_check_wolv2(conn_get_clienttag(c))) {
                    ...
                    if (clan)
                        clanid = clan_get_clanid(clan);
                    e_off += std::snprintf(entry_buf + e_off, sizeof(entry_buf) - e_off, "%s,%u,%u", name, clanid, conn_get_addr(m));
                }
                else {
                    e_off += std::snprintf(entry_buf + e_off, sizeof(entry_buf) - e_off, "%s", name);
                }
            }
            else {
                e_off += std::snprintf(entry_buf + e_off, sizeof(entry_buf) - e_off, "%s%s", flg, name);
            }

            size_t space_needed = e_off + (first ? 0 : 1);

            if (sizeof(temp) - offset > space_needed) {
                int w = std::snprintf(temp + offset, sizeof(temp) - offset, "%s%s", (first ? "" : " "), entry_buf);
                if (w > 0) offset += w;
                first = 0;
            } else {
