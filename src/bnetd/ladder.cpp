...
extern int ladder_createxptable(const char *xplevelfile, const char *xpcalcfile)
{
    ...
    while (std::fgets(buffer, 256, fd1)) {
        ...
        if (std::sscanf(buffer, " %d %d %d %d %f ", &level, &startxp, &neededxp, &mingames, &lossfactor) != 5)
            continue;

        if (level < 1 || level > W3_XPCALC_MAXLEVEL) { /* invalid level */
            eventlog(eventlog_level_error, "ladder_createxptable", "got invalid level : {}", level);
            continue;
        }
    ...
    while (std::fgets(buffer, 256, fd2)) {
        ...
        if (leveldiff > (w3_xpcalc_maxleveldiff + 1)) {
            eventlog(eventlog_level_error, __FUNCTION__, "expected entry for leveldiff={} but found {} (entries must be sequential)", w3_xpcalc_maxleveldiff + 1, leveldiff);
            continue;
        }
