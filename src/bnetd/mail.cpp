...
Mailbox::Mailbox(unsigned uid_)
    :uid(uid_), path(buildPath(prefs_get_maildir())), mdir(path, false)
{
    if (!mdir) {
        try {
            createOpenDir();
        } catch (...) {
            eventlog(eventlog_level_warn, __FUNCTION__, "failed to access mail directory for uid %u (check permissions)", uid_);
        }
    }
}
...
Mailbox::size() const
{
    if (!const_cast<Mailbox*>(this)->mdir) return 0;
...
Mailbox::empty() const
{
    if (!const_cast<Mailbox*>(this)->mdir) return true;
...
{
    if (!const_cast<Mailbox*>(this)->mdir) {
        INFO0("mail directory invalid");
        throw ReadError("mail directory invalid");
    }
...
{
    if (!const_cast<Mailbox*>(this)->mdir) return;
...
extern unsigned check_mail(t_connection const * c)
{
    ...
    try {
        ...
        eventlog(eventlog_level_error, __FUNCTION__, "Exception caught checking mail (permissions issue?)");
        return 0;
    }
