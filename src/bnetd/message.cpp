...
extern char * message_format_line(t_connection const * c, char const * in)
{
    ...
    for (inpos = 0, outpos = 1; inpos < inlen; inpos++)
    {
        if (in[inpos] != '%')
        {
            ...
            case 'a':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%u", accountlist_get_length());
                break;

            case 'c':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%d", channellist_get_length());
                break;

            case 'g':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%d", gamelist_get_length());
                break;
            ...
            case 'h':
                if (gethostname(&out[outpos], MAX_INC) < 0)
                {
                    ...
                    std::snprintf(&out[outpos], MAX_INC, "localhost");
                }
                outpos += std::strlen(&out[outpos]);
                break;

            case 'i':
                outpos += std::snprintf(&out[outpos], outlen - outpos, UID_FORMATF, conn_get_userid(c));
                break;
            ...
            case 'l':
            {
                ...
                tname = (conn_get_chatname(c) ? conn_get_chatname(c) : conn_get_loggeduser(c));
                if (remaining > 0) {
                    std::strncpy(&out[outpos], tname, remaining - 1);
                    out[outpos + remaining - 1] = '\0';
                }
            ...
            case 'm':
            {
                ...
                if (mails > 0)
                    outpos += std::snprintf(&out[outpos], outlen - outpos, "You have %u message(s) in your mailbox.", mails);
                else
                    outpos += std::snprintf(&out[outpos], outlen - outpos, "You have no mail.");
                break;
            }
            ...
            case 'r':
            {
                size_t remaining = outlen - outpos;
                if (remaining > 0) {
                    std::strncpy(&out[outpos], addr_num_to_ip_str(conn_get_addr(c)), remaining - 1);
                    out[outpos + remaining - 1] = '\0';
                }
                outpos += std::strlen(&out[outpos]);
                break;
            ...
            case 's':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%s", prefs_get_servername());
                break;

            case 't':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%s", tag_uint_to_str(clienttag_str, conn_get_clienttag(c)));
                break;

            case 'u':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%d", connlist_login_get_length());
                break;

            case 'v':
                outpos += std::snprintf(&out[outpos], outlen - outpos, PVPGN_SOFTWARE " " PVPGN_VERSION);
                break;
            ...
            case 'G':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%d", game_get_count_by_clienttag(conn_get_clienttag(c)));
                break;

            case 'H':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%s", prefs_get_contact_name());
                break;
            ...
            case 'N':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%s", clienttag_get_title(conn_get_clienttag(c)));
                break;
            ...
            case 'U':
                outpos += std::snprintf(&out[outpos], outlen - outpos, "%d", conn_get_user_count_by_clienttag(conn_get_clienttag(c)));
                break;
...
static int message_telnet_format(t_packet * packet, t_message_type type, t_connection * me, t_connection * dst, char const * text, unsigned int dstflags)
{
    ...
    case message_type_uniqueid:
        ...
        std::snprintf(msgtemp, std::strlen(text) + 32, "Your unique name: %s\r\n", text);
        break;
    case message_type_adduser:
        if (!me)
        {
        ...
        }
        {
            ...
            bufsize = std::strlen(tname) + 32;
            msgtemp = (char*)xmalloc(bufsize);
            std::snprintf(msgtemp, bufsize, "[%s is here]\r\n", tname);
    ...
    case message_type_join:
        ...
        else
        {
            ...
            bufsize = std::strlen(tname) + 32;
            msgtemp = (char*)xmalloc(bufsize);
            std::snprintf(msgtemp, bufsize, "[%s enters]\r\n", tname);
            conn_unget_chatcharname(me, tname);
        }
        break;
    case message_type_part:
        ...
        }
        {
            ...
            bufsize = std::strlen(tname) + 32;
            msgtemp = (char*)xmalloc(bufsize);
            std::snprintf(msgtemp, bufsize, "[%s leaves]\r\n", tname);
            conn_unget_chatcharname(me, tname);
        }
        break;
    case message_type_kick:
        ...
        }
        {
            ...
            bufsize = std::strlen(tname) + 32;
            msgtemp = (char*)xmalloc(bufsize);
            std::snprintf(msgtemp, bufsize, "[%s has been kicked]\r\n", tname);
            conn_unget_chatcharname(me, tname);
        }
        break;
    case message_type_quit:
        ...
        }
        {
            ...
            bufsize = std::strlen(tname) + 32;
            msgtemp = (char*)xmalloc(bufsize);
            std::snprintf(msgtemp, bufsize, "[%s quit]\r\n", tname);
            conn_unget_chatcharname(me, tname);
        }
        break;
    case message_type_whisper:
    ...
    case message_type_page:
            ...
            return -1; /* player is ignored */
        {
            ...
            {
                bufsize = std::strlen(tname) + 8 + std::strlen(newtext) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<from %s> %s\r\n", tname, newtext);
                xfree((void *)newtext); /* avoid warning */
            }
            else
            {
                bufsize = 16 + std::strlen(tname);
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<from %s> \r\n", tname);
            }
            if (me)
                conn_unget_chatcharname(me, tname);
        }
    ...
    case message_type_talk:
            ...
            return -1; /* player is ignored */
        {
            ...
            if ((newtext = escape_chars(text, std::strlen(text))))
            {
                bufsize = std::strlen(tname) + 4 + std::strlen(newtext) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<%s> %s\r\n", tname, newtext);
                xfree((void *)newtext); /* avoid warning */
            }
            else
            {
                bufsize = std::strlen(tname) + 8;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<%s> \r\n", tname);
            }
            if (me)
                conn_unget_chatcharname(me, tname);
        }
    ...
    case message_type_broadcast:
            ...
            return -1; /* player is ignored */
        {
            ...
            if ((newtext = escape_chars(text, std::strlen(text))))
            {
                bufsize = 16 + std::strlen(newtext) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "Broadcast: %s\r\n", newtext); /* FIXME: show source? */
                xfree((void *)newtext); /* avoid warning */
            }
            else
            {
                bufsize = 16;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "Broadcast: \r\n"); /* FIXME: show source? */
            }
        }
        break;
    case message_type_channel:
        ...
        msgtemp = (char*)xmalloc(std::strlen(text) + 32);
        std::snprintf(msgtemp, std::strlen(text) + 32, "Joining channel: \"%s\"\r\n", text);
        break;
    case message_type_userflags:
        if (!me)
        {
    ...
    case message_type_whisperack:
        ...
        }
        {
            ...
            if ((newtext = escape_chars(text, std::strlen(text))))
            {
                bufsize = std::strlen(tname) + 8 + std::strlen(newtext) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<to %s> %s\r\n", tname, newtext);
                xfree((void *)newtext); /* avoid warning */
            }
            else
            {
                bufsize = std::strlen(tname) + 8 + std::strlen(text) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<to %s> %s\r\n", tname, text);
            }
            conn_unget_chatcharname(me, tname);
    ...
    case message_type_friendwhisperack:   // [zap-zero] 20020518
        ...
        }
        {
            ...
            if ((newtext = escape_chars(text, std::strlen(text))))
            {
                bufsize = 14 + 8 + std::strlen(newtext) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<to your friends> %s\r\n", newtext);
                xfree((void *)newtext); /* avoid warning */
            }
            else
            {
                bufsize = 14 + 8 + std::strlen(text) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<to your friends> %s\r\n", text);
            }
        }
    ...
    case message_type_error:
        ...
        }
        {
            ...
            if ((newtext = escape_chars(text, std::strlen(text))))
            {
                bufsize = 8 + std::strlen(newtext) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "ERROR: %s\r\n", newtext);
                xfree((void *)newtext); /* avoid warning */
            }
            else
            {
                bufsize = 8 + std::strlen(text) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "ERROR: %s\r\n", text);
            }
        }
    ...
    case message_type_emote:
            ...
            return -1; /* player is ignored */
        {
            ...
            {
                bufsize = std::strlen(tname) + 4 + std::strlen(text) + 4;
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "<%s %s>\r\n", tname, text);
            }
            conn_unget_chatcharname(me, tname);
    ...
    case message_type_mode:
        ...
        }
        {
            ...
            bufsize = std::strlen(tname) + 32;
            msgtemp = (char*)xmalloc(bufsize);
            std::snprintf(msgtemp, bufsize, "%s change mode: %s\r\n", tname, text);
            conn_unget_chatcharname(me, tname);
        }
        break;
    default:
...
static int message_bot_format(t_packet * packet, t_message_type type, t_connection * me, t_connection * dst, char const * text, unsigned int dstflags)
{
        ...
        type != message_type_null) /* this does not apply for NULL messages */
    {
        ...
        std::snprintf(msgtemp, std::strlen(text) + 4, "%s\r\n", text);
    }
    else
        switch (type)
        {
        ...
        case message_type_uniqueid: /* FIXME: need to send this for some bots, also needed to support guest accounts */
            ...
            msgtemp = (char*)xmalloc(std::strlen(text) + 32);
            std::snprintf(msgtemp, std::strlen(text) + 32, "%u %s %s\r\n", EID_UNIQUENAME, "NAME", text);
            break;
        case message_type_adduser:
            if (!me)
            {
        ...
        case message_type_page:
                ...
                return -1; /* player is ignored */
            {
                ...
                bufsize = 32 + std::strlen(tname) + 32 + std::strlen(text);
                msgtemp = (char*)xmalloc(bufsize);
        ...
        case message_type_broadcast:
            ...
            if (dstflags&MF_X)
                return -1; /* player is ignored */
            {
                size_t bufsize = 32 + 32 + std::strlen(text);
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "%u %s \"%s\"\r\n", EID_BROADCAST, "_", text); /* FIXME: what does this look like on Battle.net? */
            }
            break;
        case message_type_channel:
            if (!text)
            ...
            {
                size_t bufsize = 32 + std::strlen(text);
                msgtemp = (char*)xmalloc(bufsize);
        ...
        case message_type_userflags:
            ...
            }
            {
                ...
                tname = conn_get_chatcharname(me, dst);
                bufsize = 32 + std::strlen(tname) + 16;
                msgtemp = (char*)xmalloc(bufsize);
        ...
        case message_type_friendwhisperack: // [zap-zero] 20020518
            ...
            {
                size_t bufsize = 32 + 16 + 32 + std::strlen(text);
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "%u %s \"your friends\" %04x \"%s\"\r\n", EID_WHISPERSENT, "WHISPER", conn_get_flags(me) | dstflags, text);
            }
            break;

        case message_type_channelfull:
            msgtemp = (char*)xmalloc(32);
            std::snprintf(msgtemp, 32, "%u \r\n", EID_CHANNELFULL); /* FIXME */
            break;
        case message_type_channeldoesnotexist:
            msgtemp = (char*)xmalloc(32);
        ...
        case message_type_info:
            ...
            {
                size_t bufsize = 32 + 16 + std::strlen(text);
                msgtemp = (char*)xmalloc(bufsize);
                std::snprintf(msgtemp, bufsize, "%u %s \"%s\"\r\n", EID_INFO, "INFO", text);
            }
            break;
