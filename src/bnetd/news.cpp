...
static void _news_insert_index(t_news_index *ni, const char *buff, unsigned len, int date_set)
{
    ...
    if (cni && curr != &news_head && cni->date == ni->date) {
        ...
        if ((lstr_get_len(&cni->body) + len + 2) > 1023)
            ...
            char *new_body = (char*)xrealloc(lstr_get_str(&cni->body), lstr_get_len(&cni->body) + len + 1 + 1);
            if (!new_body) {
                eventlog(eventlog_level_error, __FUNCTION__, "memory reallocation failed");
                return;
            }
            lstr_set_str(&cni->body, new_body);

            std::snprintf(lstr_get_str(&cni->body) + lstr_get_len(&cni->body),
                          len + 2,  // +2 for the newline and null terminator
                          "%s\n", buff);
        ...
        char *body_str = (char*)xmalloc(len + 2);  // +2 for newline and null terminator
        if (!body_str) {
        ...
        std::snprintf(body_str, len + 2, "%s\n", buff);
        lstr_set_str(&ni->body, body_str);
