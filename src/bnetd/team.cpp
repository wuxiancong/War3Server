...
int teamlist_load(void)
{
    ...
    if (storage->load_teams(_cb_load_teams) < 0) {
        eventlog(eventlog_level_warn, __FUNCTION__, "storage->load_teams failed (check SQL structure), using empty teamlist");
    }

    if (!teamlist_head) {
        eventlog(eventlog_level_error, __FUNCTION__, "teamlist_head is NULL, forcing creation");
        teamlist_head = list_create();
    }
...
int team_set_saveladderstats(t_team * team, unsigned int gametype, int result, unsigned int opponlevel, t_clienttag clienttag)
{
    ...
    if (ladderList) {
        ...
        eventlog(eventlog_level_warn, __FUNCTION__, "could not find ladder list for AT team update");
    }
