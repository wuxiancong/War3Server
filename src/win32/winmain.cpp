...
static void guiThread(void *param)
{
    ...
    wc.hCursor = LoadCursorW(nullptr, (LPCWSTR)IDC_ARROW);  // Cast to LPCWSTR
...
static BOOL guiOnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
    ghwndConsole = CreateWindowExW(
        0,
        L"RichEdit20W",  // Use wide string version instead of RICHEDIT_CLASS
        nullptr,
...
static void guiOnSize(HWND hwnd, UINT state, int cx, int cy)
{
    if (state == SIZE_MINIMIZED)
    {
        ...
        dta.hIcon = LoadIconW(GetWindowInstance(hwnd), MAKEINTRESOURCEW(IDI_ICON1));
...
static BOOL guiOnSetCursor(HWND hwnd, HWND hwndCursor, UINT codeHitTest, UINT msg)
{
    ...
    if (hwnd == hwndCursor && codeHitTest == HTCLIENT)
    {
        ...
        if (PtInRect(&gui.rectHDivider, p))
            SetCursor(LoadCursorW(0, (LPCWSTR)IDC_SIZENS));
...
{
    DialogBoxW(GetModuleHandleW(nullptr), MAKEINTRESOURCEW(IDD_ANN), hwnd, AnnDlgProc);
}
...
static void guiOnUserStatusChange(HWND hwnd)
{
    ...
    DialogBoxW(GetModuleHandleW(nullptr), MAKEINTRESOURCEW(IDD_KICKUSER), hwnd, KickDlgProc);
